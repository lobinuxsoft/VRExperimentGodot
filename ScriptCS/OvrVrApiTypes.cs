...
// A geographic region authorized for certain hardware and content.
	...
	enum OvrProperty
	{
		...
		VRAPI_ACTIVE_INPUT_DEVICE_ID =
			24, //< Used by apps to query which input device is most 'active' or primary, a# -1 means no active input device

		VRAPI_DEVICE_EMULATION_MODE =
			29, //< Used by apps to determine if they are running in an emulation mode. Is a OvrDeviceEmulationMode value
	}
	...
	enum OvrSystemStatus
	{
		...
		VRAPI_SYS_STATUS_USER_RECENTER_COUNT =
			15, //< Returns the current HMD recenter count for user initiated recenters only. Defaults to 0.
